...
// Utimesns set file access and modification times on a path resolved to the
...
func Utimesns(path string, times *[2]syscall.Timespec, symlinkFollow bool) error {
	err := utimens(path, times, symlinkFollow)
	return UnwrapOSError(err)
}

// UtimesnsFile is like Utimesns, except it works on a file, not a path.
//
...
func UtimesnsFile(f fs.File, times *[2]syscall.Timespec) error {
	if f, ok := f.(fdFile); ok {
