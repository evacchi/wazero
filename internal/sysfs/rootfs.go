...
func (c *CompositeFS) Utimesns(path string, times *[2]syscall.Timespec, symlinkFollow bool) error {
	matchIndex, relativePath := c.chooseFS(path)
	return c.fs[matchIndex].Utimesns(relativePath, times, symlinkFollow)
}
