...
func TestValidateConstExpression(t *testing.T) {
	t.Run("invalid opcode", func(t *testing.T) {
		...
		err := validateConstExpression(nil, 0, &expr, valueTypeUnknown, untypedFuncRefResolver)
		require.Error(t, err)
	})
	for _, vt := range []ValueType{ValueTypeI32, ValueTypeI64, ValueTypeF32, ValueTypeF64} {
		t.Run(ValueTypeName(vt), func(t *testing.T) {
			t.Run("valid", func(t *testing.T) {
				...
				err := validateConstExpression(nil, 0, &expr, vt, untypedFuncRefResolver)
				require.NoError(t, err)
			})
	...
	t.Run("ref types", func(t *testing.T) {
		t.Run("ref.func", func(t *testing.T) {
			...
			err := validateConstExpression(nil, 10, &expr, ValueTypeFuncref, untypedFuncRefResolver)
			require.NoError(t, err)
			err = validateConstExpression(nil, 2, &expr, ValueTypeFuncref, untypedFuncRefResolver)
	...
	t.Run("global expr", func(t *testing.T) {
		...
		t.Run("global index out of range", func(t *testing.T) {
			...
			err := validateConstExpression(globals, 0, &expr, valueTypeUnknown, untypedFuncRefResolver)
			require.Error(t, err)
		})
		...
		t.Run("type mismatch", func(t *testing.T) {
			for _, vt := range []ValueType{
				...
				t.Run(ValueTypeName(vt), func(t *testing.T) {
					...
					err := validateConstExpression(globals, 0, &expr, vt, untypedFuncRefResolver)
					require.Error(t, err)
				})
			}
		})
